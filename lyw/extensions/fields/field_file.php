...
class field_file
{
	...
	public function on_form($field, $default= '', $has_tip = TRUE)
	{
		...
		if ($default)
		{
			//echo '<p>当前文件：<a target="_blank" href="'.str_replace("/admin/", "", base_url()).'/'.setting('attachment_dir').$default.'">'.$default.'</a></p>';
			$filename=str_replace("/admin/", "", base_url()).'/'.setting('attachment_dir').$default;
			$extName=explode(".",$default);
			if($extName[(sizeof($extName)-1)]=="jpg"||$extName[(sizeof($extName)-1)]=="png"){
				$showName='<img src="'.$filename.'" style="width:200px;">';
			...
			echo '<p>'.$showType.'<a target="_blank" href="'.$filename.'">'.$showName.'</a></p>';
		}
	}
			...
			if ($extension_valid)
			{
				...
				if ($CI->platform->file_upload($_FILES[$field['name']]['tmp_name'], $target_file))
				{
					...
					if ($aid = $CI->db->insert_id())
					{
						...
						//表单赋值
						$post[$field['name']] = '/'.$upload['folder'].'/'.$upload['name'].'.'.$upload['type'];
					}
				}
			}	
		}		
